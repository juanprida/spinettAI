...
class LyricsDataset(Dataset):
    ...
    def __init__(self, mode, block_size=1024):
        ...
        if mode not in ["train", "validation", "inference"]:
            raise ValueError(
                "Expected train, validation or inference as mode."
            )
        if self.mode == "train":
            self.data = np.memmap(
                "data/pretrain_train.bin", dtype=np.uint16, mode="r"
            )
        elif mode == "validation":
            self.data = np.memmap(
                "data/pretrain_val.bin", dtype=np.uint16, mode="r"
            )
...
eval_loader = DataLoader(
    eval_dataset, batch_size=batch_size, shuffle=True, drop_last=True
)
...
for epoch in range(epochs):
    ...
    for i, (X, y) in enumerate(train_loader):
        ...
        outputs = model(X)
        loss = (
            criterion(outputs.view(-1, outputs.size(-1)), y.view(-1))
            / accumulation_steps
        )
    ...
    with torch.no_grad():
        for X_eval, y_eval in eval_loader:
            ...
            eval_loss += criterion(
                eval_outputs.view(-1, eval_outputs.size(-1)), y_eval.view(-1)
            ).item()
    ...
    torch.save(model.state_dict(), "pretrain/model.ckpt")
